...
sql="""select user_id,sum(recharge_gold) as gold
from stats_user_recharge_v1
where recharge_gold>0
and create_time_day>='2018-04-01' and create_time_day<='2018-04-26'
group by user_id
order by user_id"""

cur.execute(sql)
result=cur.fetchall()
df=pd.DataFrame(result,columns=['user_id','recharge'])
# print(df)
...
# print (new[['user_id','nickname','recharge']])

new.to_excel('recharge2.xlsx', u'recharge', float_format='%.5f',index=False)
